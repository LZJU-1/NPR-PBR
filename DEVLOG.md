...
上一版水滴使用 UV 作为运动坐标，生命周期和淡入淡出正确，但复杂衣服 UV 岛方向不一致，导致水珠看起来会沿不同贴图方向到处流动。随后改成世界 Y 轴竖直下流，方向统一了，但在角色身体曲面上会像一张竖直投影贴过去，弧面不够合理。

当前方案使用一个更像游戏 shader trick 的做法：把世界重力方向 `float3(0, -1, 0)` 投影到当前像素的表面切平面上，得到 `surfaceDownWS`。水滴形状仍用 UV 保持材质连续性，UV 坐标再根据 `surfaceDownWS` 在 tangent / bitangent 上的投影旋转，让滑落方向尽量贴合表面下行方向。

同时加入 `verticalSurface` 权重，接近竖直的表面更容易出现下流雨滴，接近水平的表面只保留非常弱的挂水亮点。后续尝试过世界空间双平面投影和单独 glint 通道，但在当前角色上水珠尺寸与形体可读性反而下降，因此回退到 tangent-space flow 版本。
